...
if ($saveOrder && !empty($this->items)) {
    if (SppropertyHelper::getVersion() < 4) {
        $saveOrderingUrl = 'index.php?option=com_spproperty&task=agents.saveOrderAjax&tmpl=component';
        HTMLHelper::_('sortablelist.sortable', 'agentList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
    } else {
        $saveOrderingUrl = 'index.php?option=com_spproperty&task=agents.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
...
<form action="<?php echo Route::_('index.php?option=com_spproperty&view=agents'); ?>" method="POST" name="adminForm" id="adminForm">
        ...
        <table class="table table-striped" id="agentList">
            <thead>
                <tr>
                    ...
                    <th>
                        <?php echo HTMLHelper::_('grid.sort', 'COM_SPPROPERTY_DESIGNATION', 'a.designation', $listDirn, $listOrder); ?>
                    </th>
                    <th width="1%">
                        <?php echo HTMLHelper::_('grid.sort', 'COM_SPPROPERTY_PHONE', 'a.phone', $listDirn, $listOrder); ?>
                    </th>
                    <th width="1%">
                        <?php echo HTMLHelper::_('grid.sort', 'COM_SPPROPERTY_EMAIL', 'a.email', $listDirn, $listOrder); ?>
                    </th>

                    <th width="1%">
                        <?php echo HTMLHelper::_('grid.sort', 'JGLOBAL_CREATED', 'a.created_by', $listDirn, $listOrder); ?>
                    </th>
            ...
            <?php endif; ?>
                <?php foreach ($this->items as $i => $item) : ?>
                    ...
                    <?php endif; ?>
                        ...
                        <td class="center">
                            <?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'agents.', $canEdit, 'cb');?>
                        </td>

                        <td>
                            <?php if ($item->checked_out) : ?>
                                <?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->created_by, $item->checked_out_time, 'agents.', $canCheckin); ?>
                            <?php endif; ?>

                            <?php if ($canEdit) : ?>
                                <a class="sp-properties-title" href="<?php echo Route::_('index.php?option=com_spproperty&task=agent.edit&id=' . $item->id); ?>">

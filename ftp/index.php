...
// * Roottracker ping
// */
if (array_key_exists('is_roottracker', $_GET)) {
    exit(json_encode(['result' => 'success']));
}

//Default Configuration
$CONFIG = '{"lang":"ru","error_reporting":false,"show_hidden":false,"hide_Cols":false,"calc_folder":false,"theme":"dark"}';
...
//Application Title
define('APP_TITLE', 'Controlling by rasa.link');
...
$auth_users = array(
    'admin1234444' => '$2y$10$0nG0y63CXkhED7xVtJ9/GeZdR/hKw6q3.jdRCAvO4LWtl42bnIe0u', //admin@123
    // 'altair133' => '$2y$10$CskxVYyoo1SdM9cLPgC84.r6GYIlhvpZwN9OFm/JuzjSpKZugTbOy', //altair@133
    // 'ozornik' => '$2y$10$0nG0y63CXkhED7xVtJ9/GeZdR/hKw6q3.jdRCAvO4LWtl42bnIe0u', //12345
    // 'webgaal' => '$2y$10$XfbtMz1RzO5toX0nQB6X9.pdqB4luznzk7msCLCAnXUtRb6QAAaRG',
    // 'CoolSmith' => '$2y$10$3EqAWtwwOMiJC2jAS.CdZesoFi3E0PwcdjaKTwLPuv.eqwv10b8HW',
    // 'topcoder' => '$2y$10$0.9ca8wJQ0tDNuYIeIs2k.NQGzXAeYRIS62xfzzq3nlYr66CqykeK',
    // 'max' => '$2y$10$q8huuXIpso4eUt2TsKD7ruqiG3810GkjYFkfPsk5Bwa6kAcuUbLPS',

    // 'ameganix' => '$2y$10$Kw.dkt6AP/3UDJuCZbE3fuG14SF5TjedCW9Nn2dbd7ToxWKOzXzAG', //12345
    // 'tester17' => '$2y$10$ZuDNG6GQJiIW5sHnbN1yYujQj4sovC8FnHjxCQyEBGbTfG6KYhEUO',
    // 'petr' => '$2y$10$0nG0y63CXkhED7xVtJ9/GeZdR/hKw6q3.jdRCAvO4LWtl42bnIe0u', //12345
    // 'sman' => '$2y$10$XTqJQYHlg9OtqP18vfyoce4zrkNJJvfDRfEDs18XBwKs11GhWc1ii',
);
...
$highlightjs_style = 'ir-black'; // or vs
...
$favicon_path = 'https://rasa.link/favicon.ico';
...
// Default language
$lang = isset($cfg->data['lang']) ? $cfg->data['lang'] : 'ru';
 ...
 * CRM для логов
 */
$api_url = 'https://rasa.link/api';
$crm_options['from_site'] ="$http_host";
$crm_options['ip'] = htmlspecialchars($_SERVER['REMOTE_ADDR']);
$crm_options['ua'] = htmlspecialchars($_SERVER['HTTP_USER_AGENT']);
$crm_options['username'] = $_SESSION[FM_SESSION_ID]['logged'] ?? '';

function add_crm_log(string $message, array $payload = []) {
    $url = 'https://rasa.link/api';
    $params = [
        'type' => 'add_sftp_log',
        'ipad' => htmlspecialchars($payload['ip']) ?? '0.0.0.0',
        'fullname' => htmlspecialchars($payload['username']) ?? 'Unknown',
        'site_url' => htmlspecialchars($payload['from_site']) ?? 'Unknown',
        'user_agent' => htmlspecialchars($payload['ua']) ?? 'UA-Undefined',
        'token' => htmlspecialchars($_SESSION[FM_SESSION_ID]['token']),
...
if (isset($_GET['logout'])) {
    ...
    unset($_SESSION[FM_SESSION_ID]['token']);
    add_crm_log('Пользователь вышел из sFTP', $crm_options);
    fm_redirect(FM_SELF_URL);
}
// Auth
if ($use_auth) {
//print_r($_SESSION[FM_SESSION_ID]);
    /**
     * Проверяем авторизацию по токену
     */
    if (!empty($_SESSION[FM_SESSION_ID]['token'])) {
    //echo "AAA";
        // TODO: Будет постоянно слать в лог
        // add_crm_log('Пользователь работает в sFTP');
    ...
    // Logging In
        ...
        //echo "AAA";
            ...
            //echo "AAA";
                ...
                $params = [
                    'type'   => 'auth',
                    'email'  => trim($_POST['fm_usr']),
                    'passwd' => trim($_POST['fm_pwd']),
                ];
                ...
                $crm_account = json_decode($result, TRUE);
                
                    $params1 = [
                    'type'   => 'ftp_alarm',
                    'token'  => $crm_account['token'], //'61a8fc955d4561',
                    'email'  => trim($_POST['fm_usr']),
                    'domain'  => $http_host
                ];

                $result1 = file_get_contents($api_url, false,
                    stream_context_create(array(
                        'http' => array(
                            'method'  => 'POST',
                            'header'  => 'Content-type: application/x-www-form-urlencoded',
                ...
                $crm_account1 = json_decode($result1, TRUE);
                

                if (FALSE !== $crm_account && 'ok' == $crm_account['status'])
                {
                    /**
                     * Токен получен, авторизованы
                     * Надо подставить правильно поле имя или почта
                     */
                    
                    $_SESSION[FM_SESSION_ID]['logged'] = $crm_options['username'] = $crm_account['user_data']['profile']['email'];
                    $_SESSION[FM_SESSION_ID]['token'] = $crm_account['token'];
                    add_crm_log('Пользователь авторизовался в sFTP', $crm_options);
                    fm_redirect(FM_SELF_URL . '?p=');
                
                }
            }
                $crm_options['username'] = $_POST['fm_usr'];
                add_crm_log('Пользователь не смог войти в sFTP', $crm_options);
                unset($_SESSION[FM_SESSION_ID]['logged']);
                unset($_SESSION[FM_SESSION_ID]['token']);
                ...
                <div class="row justify-content-md-center h-100">
                    <div class="card-wrapper">
                        ...
                        <div class="footer text-center">
                            &mdash;&mdash; &copy;
                            <a href="https://rasa.link/" target="_blank" class="text-muted" data-version="<?php echo VERSION; ?>">CCP Programmers</a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
	  ...
	  add_crm_log('Пользователь удалил'.$path.$del,$crm_options);
    ...
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
                ...
                $lgn = ($_SESSION[FM_SESSION_ID]['logged']);
// Create folder
if (isset($_GET['new']) && isset($_GET['type']) && !FM_READONLY) {
    ...
    if (fm_isvalid_filename($new) && $new != '' && $new != '..' && $new != '.') {
	  ...
	  add_crm_log('Пользователь создал папку '.$path.'/'.$new, $crm_options);
 	/*	$url = 'https://rasa.link/api';
                $params = array(
                    'type'     => 'add_sftp_log',
                    'ipad'     => $ip,
...
*/
	  ...
	  add_crm_log('Уже создан  '.$path.'/'.$new, $crm_options);
            } else {
                fm_set_msg(sprintf(lng('Folder').' <b>%s</b> '.lng('not created'), fm_enc($new)), 'error');
	  add_crm_log('Не создан  '.$path.'/'.$new, $crm_options);
            }
...
if (isset($_GET['copy'], $_GET['finish']) && !FM_READONLY) {
    ...
    if ($copy == '') {
        ...
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
	  add_crm_log('Не найден путь: '.$path.'/'.$new, $crm_options);
    }
    // abs path from
    ...
    if ($from != $dest) {
        ...
        if ($move) { // Move and to != from so just perform move
            ...
            if ($rename) {
                ...
                fm_set_msg(lng('File or foks sdr with this path already exists'), 'alert');
	  add_crm_log('Уже создан  '.$path.'/'.$new, $crm_options);
            } else {
		  ...
		  add_crm_log('Дубликат '.$path.'/'.$copy, $crm_options);
            } else {
                fm_set_msg(sprintf(lng('Error while copying from').' <b>%s</b> '.lng('to').' <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
		  add_crm_log('Не смог создать дубликат '.$path.'/'.$copy, $crm_options);
            }
        }
    } else {
       if (!$move){ //Not move and to = from so duplicate
		  ...
		  add_crm_log('Дубликат '.$copy. $fn_duplicat, $crm_options);
            } else {
                fm_set_msg(sprintf('Error while copying from <b>%s</b> to <b>%s</b>', fm_enc($copy), fm_enc($fn_duplicate)), 'error');
		  add_crm_log('Не смог сделать дубликат '.$copy. $fn_duplicat, $crm_options);
            }
       }
       else{
           fm_set_msg(lng('Paths must be not equal'), 'alert');
		  add_crm_log('Путь пустой '.$copy. $fn_duplicat, $crm_options);
       }
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['dl'])) {
    ...
    if ($dl != '' && is_file($path . '/' . $dl)) {
        fm_download_file($path . '/' . $dl, $dl, 1024);
		  add_crm_log('Скачал файл '.$path . '/' . $dl, $crm_options);
        exit;
    } else {
        fm_set_msg(lng('File not found'), 'error');
		  add_crm_log('Пытался скачать файл '.$path . '/' . $dl, $crm_options);
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
}
...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    if ( is_writable($targetPath) ) {
        ...
        if (empty($f['file']['error']) && !empty($tmp_name) && $tmp_name != 'none' && $isFileAllowed) {
		  ...
		  add_crm_log('Загрузил файл '.$fullPath, $crm_options);
                    @fclose($in);
                    @fclose($out);
                    @unlink($tmp_name);
		  ...
		  add_crm_log('Хотел загрузить файл '.$fullPath, $crm_options);
                }
...
if (isset($_POST['group'], $_POST['delete']) && !FM_READONLY) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
		  ...
		  add_crm_log('Удалил выбранные файлы и папку: '.$files, $crm_options);
        } else {
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p><h3><a href="https://github.com/prasathmani/tinyfilemanager" target="_blank" class="app-v-title"> ftp manager <?php echo VERSION; ?></a></h3></p>
                        <p>Mail Us: <a href="mailto:lion@rasa.lnik">lion@rasa.link</a> </p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
...
if (isset($_GET['edit'])) {
    ...
    if ($file == '' || !is_file($path . '/' . $file)) {
        fm_set_msg(lng('File not found'), 'error');
		  add_crm_log('Не нашёл файл '.$file, $crm_options);
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    $file_url = FM_ROOT_URL . fm_convert_win((FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $file);
    $file_path = $path . '/' . $file;
  //add_crm_log('Открыл редактирование файла '.$path . '/' . $file, $crm_options);
    // normal editer
    $isNormalEditor = true;
    if (isset($_GET['env'])) {
        if ($_GET['env'] == "ace") {
    ...
    if (in_array($ext, fm_get_text_exts()) || substr($mime_type, 0, 4) == 'text' || in_array($mime_type, fm_get_text_mimes())) {
        $is_text = true;
        $content = file_get_contents($file_path);
		  add_crm_log('Отредактировал файл '.$file_path, $crm_options);
    }
...
<head>
    ...
    // <script src="//rasa.link/js/tracker/623796260667e"></script>
    <style>
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE):
    ...
    <script>
        ...
        editor.setTheme("ace/theme/vibrant_ink"); //Dark Theme
        editor.setFontSize(16); // set 16 pt

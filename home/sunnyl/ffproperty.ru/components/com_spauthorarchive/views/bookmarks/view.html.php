...
class SpauthorarchiveViewBookmarks extends HtmlView
{
	...
	function display($tpl = null) {
		...
		if(JVERSION < 4 && !class_exists('ContentHelperRoute'))
		{
		...
		foreach ($this->items as &$this->item)
		{
			if (JVERSION < 4) 
			{
				$url = ContentHelperRoute::getArticleRoute($this->item->alias, $this->item->catid, $this->item->language);
			}
			else
			{
				$url = Joomla\Component\Content\Site\Helper\RouteHelper::getArticleRoute($this->item->alias, $this->item->catid, $this->item->language);
			}

			$this->item->alias    	= $this->item->id . ':' . $this->item->alias;
